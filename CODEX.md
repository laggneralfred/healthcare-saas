...
- Financial Reports v1 uses collected payment data from `checkout_payments.paid_at` as the date basis. It is a basic practice/bookkeeping export layer, not full accounting: no tax model, refunds, processor fees, or QuickBooks/Xero integration yet. Dedicated financial exports include summary, payment, and line-item CSVs and intentionally exclude clinical notes.
...
- Self-serve trial signup remains immediate: successful `/register` submissions still create the practice, owner user, and trial account right away. Each successful trial registration also creates a `trial_signups` record, sends a notification to `TRIAL_SIGNUP_NOTIFICATION_EMAIL` with fallback to `MAIL_FROM_ADDRESS`, and is visible to super-admins at `/admin/signedup`. This is notification/visibility only, not an approval gate.
- Global owner/admin access is explicit: a user with `practice_id = null` is treated as the Practiq owner/global admin and can access global resources like `/admin/signedup`. Use `php artisan practiq:make-owner-admin email@example.com` to create or promote that user safely; this does not add a broader role system or change practice-user access.
- Guided trial setup creates editable starter defaults for new trial practices: one practitioner for the registering user, Monday-Friday 9-5 working hours, Initial Visit and Follow-up Visit treatment types, matching starter service fees, and practitioner/treatment compatibility. Registrations now land on `/onboarding`, which summarizes these defaults and links directly to practitioner, working hours, treatment types, website links, HIPAA/BAA acknowledgement, AI disclaimer, and Today. The service is idempotent and can be rerun with `php artisan practiq:seed-starter-defaults {practice_id}`; it does not create fake patients, auto-book appointments, or replace legal/AI acknowledgement gates.
...
- Patient portal appointment requests use a structured appointment type plus optional practitioner preference. The portal only shows valid active treatment/practitioner combinations; AI may later summarize fuzzy preferences, but deterministic validation controls selectable options.
- Practitioner working hours are deterministic scheduling rules stored in `practitioner_working_hours`; `day_of_week` uses `0 = Sunday`, `1 = Monday`, `2 = Tuesday`, `3 = Wednesday`, `4 = Thursday`, `5 = Friday`, `6 = Saturday`.
- Practitioner time blocks live in `practitioner_time_blocks` and override normal working hours for unavailable, vacation, sick, lunch, admin, or custom blocks. Future AI may summarize preferences or rank valid slots, but valid availability must come from deterministic scheduling services.
- Staff-created appointments now validate the full selected start/end range against practitioner working hours and time blocks on both create and edit. Appointment requests can prefill the create form and show request context, but staff still choose the final slot and must explicitly mark requests scheduled when appropriate.
- Appointment create from an appointment request also shows a deterministic practitioner schedule context card for the selected date: working windows, time blocks, and existing same-practice appointments. The card links to the Calendar with request/practitioner context preserved; it does not auto-pick or create slots.
- Available slot suggestions are deterministic convenience helpers only. `AppointmentAvailabilityService` uses appointment type duration, compatible practitioners, working hours, time blocks, and same-practice busy appointments; AI is not used for availability. Suggestions do not replace create/edit validation, do not auto-create appointments, and staff must explicitly save the appointment.
- Appointment requests with a preferred practitioner use calendar-first scheduling from Today: the action opens Calendar filtered to that practitioner and preserves request context. No-preference requests remain form-first with deterministic suggested openings until multi-practitioner calendar filtering exists.
- Practice website links use stable `/p/{practiceSlug}/...` routes from the existing practice `slug`: new-patient requests create `new_patient_interests` only, existing-patient access sends a secure portal magic link without revealing whether an email exists, and public appointment requests redirect through existing-patient access until the patient is authenticated. Practice settings show copy/paste website snippets only when a slug is present.
- Trial practices now have a guidance-only setup checklist visible on Today, Reports/Dashboard, and Settings -> Setup Checklist. It checks practice name/timezone/slug, active practitioners, active treatment types, practitioner-to-treatment compatibility, practitioner working hours, and website-link readiness. It does not block app usage. If no active practitioner/treatment compatibility exists, staff see: "Patients will not see visit types until practitioners are attached to treatment types."
- Browser-level tests use Playwright under `tests/e2e`. Run with `npm run test:e2e`; headed mode is `npm run test:e2e:headed`; interactive UI mode is `npm run test:e2e:ui`. The default base URL is `http://127.0.0.1:8002`, and the config starts `php artisan serve` locally. The suite runs with one worker to avoid shared local staff-login throttling and data collisions. Global setup creates/updates a local staff user from `E2E_ADMIN_EMAIL` / `E2E_ADMIN_PASSWORD` defaults for browser login without changing the password hash when it already matches. Do not point Playwright at production Practiq hosts.
- Current Playwright e2e map: `admin-smoke.spec.ts` covers login, Today/dashboard, sidebar navigation, Patients, Calendar, Visits, Follow-Up, Practice Settings, and logout; `patient-management.spec.ts` covers patient detail actions and edit navigation; `public-practice-links.spec.ts` covers Website Links and public slug routes; `public-new-patient.spec.ts` covers public interest submission and no automatic Patient creation; `patient-portal-appointment-request.spec.ts` covers request-only patient portal scheduling; `staff-calendar-scheduling.spec.ts` covers calendar-first preferred-practitioner scheduling; `no-preference-suggestions.spec.ts` covers form-first no-preference suggestions; `patient-portal-forms.spec.ts` covers assigned form completion and staff review without overwriting demographics; `scheduling-validation.spec.ts` covers invalid practitioner times staying unsaved and valid working-hour creation.
