...
- Browser-level smoke tests use Playwright under `tests/e2e`. Run with `npm run test:e2e`; headed mode is `npm run test:e2e:headed`; interactive UI mode is `npm run test:e2e:ui`. The default base URL is `http://127.0.0.1:8002`, and the config starts `php artisan serve` locally. Do not point Playwright at production Practiq hosts.
