...
- `BelongsToPractice` global scope (Eloquent layer) enforces tenant isolation on all practice-scoped models
- Public routes (/intake, /consent): practice resolved from the token record
- **PostgreSQL RLS is intentionally disabled** — tenant isolation is handled exclusively at the application layer via `BelongsToPractice`

### Database users

| User | Privileges | Used by |
|------|-----------|---------|
| `postgres` | Superuser | Tests (`phpunit.xml`) — avoids any future RLS friction |
| `healthcare` | App user | Web app (`.env`), `migrate:fresh --seed` in dev |
